  ...
  async ({ taskDescription, reasoning }) => {
    ...
    try {
      ...
      if (confirmed) {
        ...
        return `NEW TASK CONFIRMED: ${taskId} - "${taskDescription}". Starting execution.`;
      } else {
        const feedbackRl = readline.createInterface({
          input: process.stdin,
          output: process.stdout,
        });
        const feedback = await feedbackRl.question(
          `\n❓ What should the task be instead? `
        );
        feedbackRl.close();

        return `TASK FEEDBACK: "${feedback}" - Please refine the task based on this feedback.`;
      }
    } catch (error) {
      rl.close();
      return `Unable to get task confirmation: ${error}. Proceeding with original interpretation.`;
    }
  },
  {
    name: "new_task",
...
export const askForClarification = tool(
  async ({ question, context }) => {
    ...
    try {
      ...
      if (answer.trim()) {
        return `User provided clarification: "${answer.trim()}"`;
      } else {
        return `User did not provide clarification - proceeding with best interpretation.`;
      }
    } catch (error) {
      rl.close();
      return `Unable to get user input: ${error}. Proceeding with best interpretation.`;
    }
  },
  {
...
export const confirmAction = tool(
  async ({ action, consequences }) => {
    ...
    try {
      ...
      if (confirmed) {
        return `User confirmed the action: "${action}". Proceeding.`;
      } else {
        return `User declined the action: "${action}". Cancelled.`;
      }
    } catch (error) {
      rl.close();
      return `Unable to get user confirmation: ${error}. Action cancelled for safety.`;
    }
  },
  {
    name: "confirm_action",
    description:
      "Ask for user confirmation before taking potentially risky or irreversible actions",
    schema: z.object({
      action: z.string().describe("The action that needs confirmation"),
      consequences: z
        .string()
        .describe("What will happen if this action is taken"),
    }),
  }
);
...
export const attemptCompletion = tool(
  async ({ summary, details, nextSteps }) => {
    ...
    try {
      ...
      if (satisfied) {
        return `TASK COMPLETED - User is satisfied. Summary: ${summary}. Details: ${details}.`;
      } else {
        const feedback = await readline
          ...
          .question(`\n❓ What would you like me to improve or change? `);

        return `USER FEEDBACK: "${feedback}" - Please incorporate this feedback and continue working.`;
      }
    } catch (error) {
      rl.close();
      return `Unable to get user satisfaction check: ${error}. Assuming task complete.`;
    }
  },
  {
